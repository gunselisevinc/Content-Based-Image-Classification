...
def validationlevel1(kvalue,binvalue,image_listTrain,labels):
	...
	for filename in glob.glob(filelocation):  # open all jpg files
		im3 = Image.open(filename).convert('LA').resize((500, 325))
	...
	for x in image_listValidation1:
		...
		#print("Retval : "+str(retval)+"validation : "+str(validationLabels[count]))
...
for filename in glob.glob(filelocation): # open all jpg files
	im = Image.open(filename).convert('LA').resize((500, 325))
...
for filename in glob.glob(filelocation): # open all jpg files
	im2 = Image.open(filename).convert('LA').resize((500, 325))
